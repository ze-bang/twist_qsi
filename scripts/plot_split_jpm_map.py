#!/usr/bin/env python3
"""6x3 comparison grid with Jpm=-0.1 split from smaller-amplitude sweeps."""
...
def plot_overlay_row(axes_row, title: str, cases: list[tuple[str, dict]], t_min: float) -> None:
    ...
    for i, (label, case) in enumerate(cases):
        ...
        for op, op_label in EG_OPS:
            ax_eg.plot(case["T"], case["alpha"][op], color=color, lw=2.0, ls=eg_styles[op], label=f"{label} {op_label}")
        for op, op_label in T2G_OPS:
            ax_t2g.plot(case["T"], case["alpha"][op], color=color, lw=2.0, ls=t2g_styles[op], label=f"{label} {op_label}")
...
def main() -> None:
    ...
    ap.add_argument("--eg-jpm-highlight", required=True, help="single LABEL=PATH Eg-source case to isolate")
    ap.add_argument("--t2g-jpm", action="append", default=[], help="repeat LABEL=PATH for T2g-source Jpm sweep")
    ap.add_argument("--t2g-jpm-highlight", required=True, help="single LABEL=PATH T2g-source case to isolate")
    ...
    eg_highlight_label, eg_highlight_path = parse_case_spec(args.eg_jpm_highlight)
    eg_highlight = (eg_highlight_label, load_case(eg_highlight_path))
    t2g_cases = [(label, load_case(path)) for label, path in (parse_case_spec(spec) for spec in args.t2g_jpm)]
    t2g_highlight_label, t2g_highlight_path = parse_case_spec(args.t2g_jpm_highlight)
    t2g_highlight = (t2g_highlight_label, load_case(t2g_highlight_path))

    t_min = float(np.min(baseline["T"][baseline["T"] > 0]))
    fig, axes = plt.subplots(6, 3, figsize=(16, 20), sharex=True)

    plot_single_row(axes[0], r"Baseline: $J_3=0$", baseline, t_min)
    plot_single_row(axes[1], r"Clean: $J_3=0.08$", j3_08, t_min)
    plot_overlay_row(axes[2], r"$E_g$ source: $-10^{-3}Q_1$ ($J_{\pm}=-0.3,-0.2$)", eg_cases, t_min)
    plot_overlay_row(axes[3], r"$E_g$ source: $-10^{-3}Q_1$ ($J_{\pm}=-0.1$)", [eg_highlight], t_min)
    plot_overlay_row(axes[4], r"$T_{2g}$ source: $-10^{-3}Q_{xz}$ ($J_{\pm}=-0.3,-0.2$)", t2g_cases, t_min)
    plot_overlay_row(axes[5], r"$T_{2g}$ source: $-10^{-3}Q_{xz}$ ($J_{\pm}=-0.1$)", [t2g_highlight], t_min)

    for col in range(3):
        axes[-1, col].set_xlabel(r"$T\,/\,|J_{zz}|$")

    fig.suptitle(
        r"Exact full-ED thermal expansion and specific heat with split-out $J_{\pm}=-0.1$ source responses",
        fontsize=13,
    )
    fig.tight_layout()
