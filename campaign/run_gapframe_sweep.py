#!/usr/bin/env python3
"""Winding-free construction on the sub-charge-gap manifold.
...
OUTPUT = ROOT / "campaign" / "outputs" / "gapframe_sweep"
...
def main() -> None:
    ...
    for jpm in couplings:
        ...
        tag = f"{jpm:+.3f}".replace(".", "p")
        np.savez_compressed(
            OUTPUT / f"gapframe_{tag}.npz",
            jpm=jpm, rank=rank, cutoff=cutoff,
            window_margin=window_margin,
            raw_selected=energies, gap_band=gap_band,
            gap_peak=band_peak(gap_band),
