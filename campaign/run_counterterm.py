#!/usr/bin/env python3
"""Winding-4-free ED at arbitrary coupling via a nonperturbative counterterm.
...
OUTPUT = ROOT / "campaign" / "outputs" / "counterterm"
...
def main() -> None:
    ...
    pair_a, pair_b = winding_pair(cluster, index, cluster.loops4[0][0])
    ...
    for jpm in couplings:
        ...
        c = 4.0 * jpm * jpm      # perturbative seed
        history = []
        for iteration in range(20):
            h_c = (h0 + c * winding_ops).tocsc()
            amplitude = exact_winding_amplitude(
                h_c, pair_a, pair_b, z, complement)
            history.append((c, amplitude))
            if abs(amplitude) < 1e-10:
                break
            c = c - amplitude    # Newton step with slope ~ 1 (bare term)
        ...
        ground = psi[:, 0]
        flux = np.mean([float(np.real(ground.conj() @ (op @ ground)))
                        for op in hex_ops])
        ...
        np.savez_compressed(
            ...
            jpm=jpm, c=c, history=np.asarray(history),
            levels=levels, band_peak=peak, gs_flux=flux)
        g6 = 12.0 * abs(jpm) ** 3
        print(f"jpm={jpm:+.3f} c={c:+.6f} (4l2={4*jpm*jpm:.6f}) "
              f"iters={len(history)} peak/g6={peak/g6:.4f} "
              f"GSflux={flux:+.4f} ({time.perf_counter()-started:.0f}s)",
              flush=True)
