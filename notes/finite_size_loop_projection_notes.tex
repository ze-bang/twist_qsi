...
transported dipoles are half-cell displacements.  For the order-$\Jpm^2$ and
order-$\Jpm^3$ ice-manifold rows, the natural integer coordinate is therefore
\begin{equation}
\bDelta_r\equiv2\bdelta_r .
\label{eq:delta2_def}
\end{equation}
The offending rows have components $\Delta_\mu=0,\pm1$, whereas the clean
hexagons have $\bDelta=0$.  An ordinary physical flat holonomy is not the same
as this row-character projector.  This is why physical twist averaging
suppresses the artifact but does not reproduce the strict $\bdelta=0$
projector.

The exact projector needs a compact character variable with $2\pi$ periodicity:
\[
...
$2\pi$-periodic $\theta$ circle.  Multiplying by $2$ converts the transported
dipoles appearing through third order into integer labels.  Since
multiplication by $2$ is invertible as a test of zero, it changes no physics in
the projection: $\bDelta=0$ if and only if $\bdelta=0$ for these rows.

The exact character variable for the finite cluster is therefore not the
integer winding $\bw$, but the integer transported dipole $\bDelta=2\bdelta$:
\begin{equation}
...
transported-dipole increment is $-\bd_{ij}$.  The deformation used in the
adopted $2\bdelta$ protocol is therefore
\begin{equation}
\Hmic^{2\delta}(\btheta)=
\Jzz\sum_{\langle ij\rangle}S_i^zS_j^z
-\Jpm\sum_{\langle ij\rangle}
...
\end{equation}
This is not an ordinary physical flux insertion.  It is an auxiliary source
field chosen so that the relevant ice-manifold virtual processes are filtered
by the $2\bdelta$ character.  Physical flat twists are still useful
diagnostics, but Eq.~\eqref{eq:mic_dipole_twisted} is the deformation used in
the $M=2$ full-Hamiltonian protocol below.

Let $\{|\psi_n(\btheta)\rangle\}_{n=1}^{N_{\ice}}$ be the lowest ice-like band
of $\Hmic^{2\delta}(\btheta)$, with energies $E_n(\btheta)$.  Let
\[
...
\end{equation}
For the $16$-site campaign below we use $\bDelta=2\bdelta$.  The spurious
four-loop sectors then carry $\Delta_\mu=\pm1$, so the two-point grid $M=2$
distinguishes them from zero.  This is the smallest character average that
exactly reproduces the strict $\bdelta=0$ row projector through order $\Jpm^3$.
...
$\{|\alpha\rangle\}$.
\item For each character field $\btheta$ on the grid, build
$\Hmic^{2\delta}(\btheta)$ in the full fixed-$S^z_{\rm tot}=0$ Hilbert space
using Eq.~\eqref{eq:mic_dipole_twisted} and the $M=2$ grid
$\theta_\mu\in\{0,\pi\}$.
...
The full microscopic QED run used Eq.~\eqref{eq:mic_dipole_twisted} on the
$M=2$ grid, i.e. $8$ source-field points.  The low-band pullback was well
conditioned:
\[
...
$0.2612$.  Thus the working protocol for the present $16$-site campaign is the
perturbatively minimal $2\bdelta$, $M=2$ character average.  It is a large
improvement over the ordinary physical $M=2$ holonomy average, whose operator
error against $\bdelta=0$ was $5.4308$.
...
\subsection{All-temperature check: keep the full spectrum outside the ice band}

The preceding comparison is deliberately a low-energy comparison.  It uses the
lowest $N_{\rm ice}=90$ eigenvectors of the microscopic Hamiltonian, pulls that
band back to the ice basis, and then applies the $2\bdelta$, $M=2$ character
projection.  That construction cannot, by itself, contain the other specific
heat peak at larger temperature: the larger-$T$ peak is produced by states
outside the ice manifold, namely sectors with violated tetrahedron constraints.

The all-temperature diagnostic should therefore not be a twist average of the
entire microscopic partition function.  The phases in
Eq.~\eqref{eq:mic_dipole_twisted} are auxiliary source fields designed to
resolve transported-dipole sectors inside the ice perturbation theory.  If we
apply them to every high-energy state and then thermally average the resulting
Hamiltonians, we are no longer only removing the spurious four-site loop.  We
are also changing the spinon sectors that are responsible for the larger
specific-heat peak.

The correct full-temperature test is a spectral surgery which acts only where
the finite-size pathology lives.  Let
\[
...
\]
be the spectrum of the $2\bdelta$, $M=2$ twist-projected microscopic ice-band
operator.  We define the all-temperature corrected spectrum by
\begin{equation}
\label{eq:allT_spectral_replacement}
\widetilde E_a =
\begin{cases}
...
\end{equation}
The shift $s$ fixes the center of the ice band relative to the spinon
continuum.  It has no effect on the internal clean-loop splittings, but it
prevents the diagnostic from artificially changing the ice-to-spinon gap.  The
specific heat is then computed from the ordinary canonical expression
\[
C(T)=\frac{\langle E^2\rangle_T-\langle E\rangle_T^2}{T^2}
\]
using either $\{E^{\rm full}_a\}$ or $\{\widetilde E_a\}$.

We computed $E^{\rm full}$ locally with QED by exact full-spectrum ED: all
$2^{16}=65536$ eigenvalues were obtained by decomposing the Hamiltonian into
all conserved $S^z_{\rm tot}$ sectors and the four translation irreducible
representations of the $1\times1\times1$ cubic pyrochlore cluster.  As a
check, the lowest $90$ eigenvalues of this full spectrum agree with the
previous untwisted low-band QED extraction to
$6.22\times 10^{-14}$.
...
Thus the correction strongly changes the low-temperature peak, where the
finite-size four-loop lives, but it leaves the larger-temperature full-ED peak
essentially fixed: the relative shift of the larger peak is
$4.37\times10^{-4}$ and the relative height change is $-4.83\times10^{-3}$.
This is the all-temperature consistency test we want.  The protocol removes the
spurious ice-band finite-size loop without erasing the microscopic
constraint-violation physics at larger temperature.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{fig_full_ed_all_temperature_specific_heat.pdf}
\caption{\textbf{All-temperature full-ED check.}  The black curve is the exact
untwisted microscopic full-ED specific heat for all $65536$ states of the
$16$-site cluster.  The green curve keeps every non-ice eigenvalue fixed and
replaces only the lowest $90$ ice-band eigenvalues by the $2\bdelta$, $M=2$
clean twist-projected band, with the band-center shift in
Eq.~\eqref{eq:allT_spectral_replacement}.  The lower panel zooms into the
gauge scale and also shows the two isolated low-band traces.  The correction
moves only the spurious low-temperature four-loop peak; the larger full-ED peak
at $T/\Jzz\simeq0.253$ remains in place.}
\label{fig:full_ed_all_temperature_specific_heat}
\end{figure}
