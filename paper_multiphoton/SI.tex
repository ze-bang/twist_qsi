...
E_0=-14.80971536K .
\end{equation}
The identification is verified by explicit diagonalization rather than
inferred, and on the two clusters where the winding sectors can be
enumerated in full the verification is exhaustive.

On the 48-site cluster all $155$ sectors were diagonalized. The
ground state lies in the sector of largest dimension ($9104$), and the
lowest energy of any other sector is higher by $0.988K$. On the
72-site cluster all $310$ sectors were diagonalized. The ground
energy is attained in the two largest sectors ($973\,008$ states
each), which are the exactly degenerate $\pm\bm P$ pair related by
spin reversal, and the lowest energy of any sector outside that pair
is higher by $0.998K$.

These exhaustive checks also establish what cannot be assumed. Sector
energy is \emph{not} monotone in sector dimension: on the 72-site
cluster a sector of dimension $401\,724$ lies at $-13.811K$, below
sectors of dimension $498\,654$ at $-13.746K$. Dimension is therefore
not a variational argument, and we do not use it as one.

At 96 sites, where $519$ sectors exist and exhaustive diagonalization
is out of reach, the twelve largest candidates were diagonalized
directly. The lowest is the sector of dimension $146\,758\,352$ used
throughout, at $E_0=-19.5679K$, with the next distinct energy higher
by $0.375K$. Across those twelve, sector energy rises steadily as the
dimension falls---from $-19.568K$ at $1.47\times10^{8}$ states to
between $-18.2K$ and $-18.6K$ at $5.5\times10^{7}$---so a sector
outside the twelve, every one of which is smaller still, would have to
be a dramatic outlier to lie lower. That is evidence rather than
proof: all quantities reported at 96 sites refer to the lowest sector
among those tested, and the exhaustive results at 48 and 72 sites,
where the largest sector does contain the ground state by an $O(1)K$
margin, are what make it the physically relevant one.
% ================================================================
...
Full reorthogonalization against all previously generated Krylov
vectors is performed at every iteration and at every cluster size; no
selective or partial scheme is used, and the Lanczos basis is retained
in memory for that purpose. The equal-time sum rule is applied as a
normalization gate, but it does not by itself certify individual
poles, so the 96-site spectra were recomputed independently at two
Krylov dimensions, $m=120$ and $m=200$, and compared line by line.

The two dominant lines at each of the two inequivalent $L$ classes ---
the levels carrying all quantities used in this paper --- are
converged to machine precision. At the reconstructing class,
\begin{equation}
\begin{array}{lccc}
 & \omega/K & Z & d\omega/dV\\[2pt]
m=120 & 0.904043 & 0.192939 & +0.25927\\
m=200 & 0.904043 & 0.192939 & +0.25927\\[4pt]
m=120 & 1.647098 & 0.079920 & +0.13465\\
m=200 & 1.647098 & 0.079920 & +0.13465
\end{array}
\end{equation}
with $|\Delta\omega|=0$ and $|\Delta Z|<5\times10^{-17}$; at the
nonreconstructing class the corresponding differences are
$|\Delta\omega|<4\times10^{-15}$ and $|\Delta Z|<8\times10^{-16}$.
The third line at each momentum, carrying between $1$ and $2\%$ of
the weight, agrees to between $10^{-11}$ and $10^{-6}$ in energy.
The ground-state energy and $\langle n^{\rm flip}\rangle$ are
identical in the two runs to all printed digits, as is $S(\bq)$.

Beyond the third line the individual Ritz values are \emph{not}
converged, and we say so: each carries less than one percent of the
weight, and their ordering differs between the two runs. This is the
expected behaviour of a Lanczos continued fraction, whose tail
represents integrated high-energy weight rather than resolved
individual poles. No statement in this paper depends on those lines;
the off-pole weight is quoted only as an aggregate.
...
\subsection{Exact degeneracy of the transverse doublet}
\label{sec:degeneracy}

A property of Eq.~(\ref{eq:gaussian}) that the Letter uses as its
reference point deserves to be stated separately, because it is what
makes the measured splitting a qualitative rather than a quantitative
statement. At every momentum of every cluster we examined, the two
nonzero singular values are not merely both present but \emph{equal}
to machine precision,
\begin{equation}
\sigma_1(\bq)=\sigma_2(\bq)
\qquad\text{for all }\bq\neq\Gamma,
\end{equation}
so the Gaussian transverse doublet is exactly degenerate throughout
the zone. Representative values on the 48-site cluster are
$\sigma=3$ at $(\frac13,\frac13,\frac13)$, $2\sqrt3=3.4641016151$ at
$L$, $\sqrt{15}=3.8729833462$ at $(\frac16,\frac16,\frac56)$, $\sqrt{13}$ at $(\frac13,\frac13,\frac23)$, and $4$ at $X$,
each appearing twice with the remaining singular values below
$10^{-15}$. The agreement between the two members is at the level of
the singular-value solver, $\lesssim10^{-15}$ relative.

The consequence is the one the Letter draws. The parameters $U$ and
$K$ enter Eq.~(\ref{eq:gaussian}) only through the overall prefactor
$\sqrt{2UK}$, which multiplies both members identically. Renormalizing
the Gaussian theory---by any amount, in either parameter, at any
momentum-independent strength---therefore rescales the doublet without
splitting it. A nonzero splitting cannot be produced within Gaussian
lattice electrodynamics at all, and its observation is not a
quantitative correction to that description but a failure of it. This
is why the branch ratio $\omega_{\rm low}/\omega_{\rm high}$ is used
in the Letter as a scale-free diagnostic: its Gaussian value is unity
for every choice of parameters, so any deviation is a pure
interaction effect, and unlike the softening
$\omega_{\rm low}/\omega_G$ it does not require the fitted scale
$\sqrt{2UK}=0.5868K$ at all.

We emphasize that the degeneracy is a statement about the Gaussian
theory on these tori, verified numerically at every momentum we use,
and not a symmetry argument we have proved in general. Nothing in the
Letter requires more than the verified statement.
...
\subsection{Why the ratio is a bound and not merely a centroid}
\label{sec:why_bound}

Equation~(\ref{eq:omegaF}) admits two readings, and only the second
carries evidential weight.

The first is that $\omega_F$ is the centroid of
$\Szz(\bq,\omega)$---the first moment divided by the zeroth. Read this
way the quantity is a summary of a spectrum, and once that spectrum
has been computed it says nothing the spectrum does not already say. A
low centroid at a momentum whose spectrum we have diagonalized is a
restatement of the low pole, not evidence for it.

The second reading is the operative one. Let
\begin{equation}
|\psi_{\bq}\rangle
=
\frac{S^z(\bq)|0\rangle}{\sqrt{S(\bq)}},
\end{equation}
which is normalized and, for $\bq\neq\Gamma$, orthogonal to the ground
state because $S^z(\bq)$ carries nonzero momentum. Then
\begin{equation}
\langle\psi_{\bq}|H-E_0|\psi_{\bq}\rangle
=
\frac{\langle0|S^z(-\bq)(H-E_0)S^z(\bq)|0\rangle}
{\langle0|S^z(-\bq)S^z(\bq)|0\rangle}
=
\frac{f_1(\bq)}{S(\bq)},
\end{equation}
the numerator being $\sum_n\omega_n|\langle n|S^z(\bq)|0\rangle|^2=f_1$
by definition. So $\omega_F$ is the variational energy of an explicit
trial state, and by the Rayleigh--Ritz principle applied within the
subspace orthogonal to $|0\rangle$,
\begin{equation}
\omega_F(\bq)\;\geq\;\omega_{\min}(\bq),
\end{equation}
where $\omega_{\min}$ is the lowest excitation at $\bq$ odd under
global spin reversal. This is a rigorous inequality, and both sides of
Eq.~(\ref{eq:omegaF}) are ground-state expectation values. A small
$\omega_F$ therefore \emph{certifies} the existence of a low-lying
odd excitation at that momentum without any excited state having been
computed. That is the sense in which the quantity is used.

The bound is not tight, and we do not present it as one. At $L$ on the
48-site cluster $\omega_F=1.846K$ against a true lowest excitation of
$1.024K$, an overestimate of $80\%$, because the centroid is pulled up
by the high-energy weight in the same channel. What survives the
looseness is the \emph{ordering}, which is the use we make of it.

\subsection{The ratio ranks the momenta correctly}
\label{sec:rank}

Because $\omega_F$ grows with $|\bq|$ for trivial reasons, momenta at
different $|\bq|$ are compared after normalizing by the Gaussian
energy $\omega_G=\sqrt{2UK}\,\sigma$ at the single fitted scale of
Sec.~\ref{sec:gaussian}. Table~\ref{tab:rank} collects every momentum
resolved at 48 sites.

\begin{table}[h]
\caption{Ground-state predictor against measured softening, all
momenta resolved at 48 sites. The predictor $\omega_F/\omega_G$ is
built entirely from ground-state data; $\omega_{\rm low}/\omega_G$ is
measured from the diagonalized spectrum. The two columns order the
five momenta identically.}
\label{tab:rank}
\begin{ruledtabular}
\begin{tabular}{lccccc}
$\bq$ & $|\bq|$ & $S(\bq)$ & $f_1(\bq)$ & $\omega_F/\omega_G$ &
...
The two orderings agree on all five entries, with no inversions: the
Spearman rank correlation is exactly $+1$. Neither column is ordered
by $|\bq|$, which is the point of including that column: the first and
last rows sit on the same shell $|\bq|=0.866$ and are separated by the
predictor ($0.908$ against $1.415$) exactly as they are separated by
the measurement ($0.503$ against $1.114$). The predictor is therefore
not a proxy for wavevector magnitude. The predictor separates the
two momenta that soften ($0.908$, $1.177$) from the three that do not
($1.373$--$1.415$) by a clear gap, and within the softening pair it
places $L$ first, which is where the reconstruction is largest. This
is the sense in which the ground-state ratio locates the
reconstruction rather than merely describing it afterwards.

Three further forward tests are reported elsewhere in this
Supplement and were made before the corresponding spectra were
computed: the discrimination between the two inequivalent $L$ classes
at 64 sites, the same discrimination at 96, and the predicted near
degeneracy of $L$ and $(\frac16,\frac12,\frac12)$ at 72 sites, where
ratios differing by $0.9\%$ produced levels differing by $0.7\%$.

We note the limits of the diagnostic. It bounds the lowest odd
excitation but says nothing about spectral weight, so it cannot by
itself distinguish a soft mode that is bright from one that is dark;
the diagonalizations supply that. It is an inequality in one direction
only, so a large $\omega_F$ does not forbid a low mode---it merely
fails to certify one. And the normalization by $\omega_G$ inherits the
single fitted scale of Sec.~\ref{sec:gaussian}, though only as a
common factor across the table, so the ordering in
Table~\ref{tab:rank} would be unchanged by any other choice of that
scale.
...
% ================================================================
\section{Large-system landscape of the Feynman bound}
\label{sec:thermo}
...
fingerprint of the reconstructing momentum class, the reconstructed
mode gaining flippable hexagons at every size and every
reconstructing momentum while every dominant level at a
nonreconstructing momentum loses them;

\item
the ground-state Feynman ratio, a rigorous variational upper bound,
ranks the resolved momenta in exactly the order of their measured
softening and discriminates the reconstructing from the
nonreconstructing classes on the finite clusters;

\item
GFMC establishes the large-system momentum dependence of that bound
up to 2048 sites.
\end{enumerate}
