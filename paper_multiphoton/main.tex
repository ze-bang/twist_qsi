...
\title{Nonperturbative zone-boundary reconstruction of the emergent photon\\ in quantum spin ice}
...
compact gauge theory is known to be strongly interacting. We show
that in the pure ring-exchange model the interactions reconstruct the
finite-momentum spectrum at order one, and that where they do so is
predicted by ground-state quantities alone. Operator-resolved
spectroscopy of the longitudinal correlation function
$\Szz(\bq,\omega)$ on 48-, 64-, 72- and 96-site pyrochlore clusters
--- complete diagonalization at 48 sites, gated Lanczos continued
fractions up to a 147-million-dimensional constrained sector at 96 ---
finds at every size a dominant level in one transverse channel at
$0.90$--$1.07K$, carrying $50$--$54\%$ of the longitudinal weight at
the zone-boundary $L$ point, at roughly half the Gaussian-photon
energy, while every other accessible momentum remains photon-like.
The location of the reconstruction is fixed by the ground-state
Feynman ratio $f_1(\bq)/S(\bq)$: on the two clusters whose anisotropy
splits the $L$ star into inequivalent classes, the ratio selects in
advance which class reconstructs, and on the 72-site cluster the
near-equality of the ratio at two different momentum stars correctly
predicts the near-degeneracy of their soft levels. Because both
factors of the ratio are ground-state quantities, sign-free
Green's-function Monte Carlo --- validated digit-for-digit against
the 48-site spectra --- carries the diagnostic to complete momentum
grids of up to 2048 sites, where the zone-boundary softening at $L$
persists with under two percent drift, together with a shallow soft
valley along $(t,\frac12,\frac12)$. An exact, basis-invariant
plaquette decomposition shows that the mode recovers a size-stable
$28$--$32\%$ of its bare single-mode energy by reorganizing ring
coherence in a strongly family-selective way, though not one
concentrated in the geometrically dark hexagon family. The
phenomenology --- a soft, weight-dominant collective level at finite
momentum, located by the Feynman ratio --- is that of a lattice
roton, an interpretation we state precisely and whose thermodynamic
limits (sharpness is established to 96 spins; beyond that the Monte
Carlo
constrains the spectral centroid) we state alongside it.
\end{abstract}
...
quantities --- to 2048 sites. What emerges is a long-wavelength photon
that survives as expected and a zone-boundary spectrum that is
strongly reconstructed. The result of this paper is the
reconstruction itself: at the zone-boundary $L$ point --- and only
where the ground-state Feynman ratio $f_1/S$ says so --- the spectrum
of one transverse channel collapses onto a dominant level at roughly
half the Gaussian-photon energy, carrying about half of the
longitudinal weight, below the dominant level at every other
accessible momentum, at every diagonalized size. The
\emph{interpretation} we offer for this phenomenology --- a lattice
analogue of a Feynman roton, softened where $S(\bq)$ is large and the
oscillator strength small --- is developed in Sec.~\ref{sec:roton-sense}, together with a
precise statement of what it does and does not commit us to. The
reconstruction stands independently of whether the reader adopts the
interpretation.
...
\caption{The $S^z$ spectrum as a dispersion relation. Every resolved
 pole of the diagonalized clusters (complete diagonalization at 48
 sites, Lanczos-converged at 64, 72 and 96, overlaid)
 ...
 the two dominant transverse-channel levels; teal: residual weight beyond the two dominant
 levels (its multiparticle versus magnetic character is discussed in
 the text). The dashed curve is the Gaussian photon,
...
succeeds again. The third is a near-degeneracy prediction: on the
72-site cluster two \emph{different} momentum stars happen to have
ratios within $0.9\%$ of one another, and their soft levels come out
within $0.7\%$. We stress what this does and does not show: the
ratio does NOT predict the absolute pole energy --- at $L$ it is
$1.98K$ against a pole at $1.07K$, since the centroid also carries
the high-lying weight --- but the near-equality of two ground-state
centroids correctly predicted the near-degeneracy of two soft
spectral features at different momenta, which no fitted quantity
entered. The fourth is the trajectory under the
...
single-mode cost --- and this recovery fraction is size-stable, $29$,
$32$ and $28\%$ on the 48-, 64- and 72-site clusters. Because the
ring term is the entire nonconstant Hamiltonian, the recovery
decomposes exactly, with no remainder, into contributions from the
four hexagon families (Appendix~\ref{app:backflow}). The
decomposition must be performed with care: the soft level is
degenerate, and the per-family coherences of an arbitrarily chosen
multiplet member are basis-dependent. The basis-invariant
construction --- decomposing the projection of the probe onto the
multiplet, which is the state the probe actually creates, and which
agrees to four decimals with the alternative alignment to the
single-mode state --- shows the relief to be strongly
family-selective but \emph{not} concentrated in the geometrically
dark family: the dark family carries $21$, $9$ and $32\%$ of the
relief at the three sizes, with the largest single-family share
($50$--$76\%$) residing in bright families whose pattern varies with
cluster geometry. An earlier version of this analysis attributed
$71\%$ to the dark family; that number was an artifact of the basis
choice within the degenerate multiplet, and we retract the
Feynman--Cohen backflow interpretation~\cite{FeynmanCohen1956} that
was built on it. What the decomposition does establish is that the
collective mode lowers its energy by reorganizing ring coherence in a
few specific hexagon families --- a correlated, family-selective
relaxation that the single-mode state cannot perform --- and that the
size of this effect is stable across every cluster we can
diagonalize.
...
\section{Interpretation: a lattice Feynman-roton phenomenology}
\label{sec:roton-sense}

Everything up to this point is a computed property of the model. This
section is interpretation, and we present it as such: the reader who
declines it loses none of the results.

The phenomenology assembled above --- a soft, weight-dominant
collective level at finite momentum, located predictively by the
ratio of oscillator strength to static structure factor, and lowered
below the single-mode bound by a size-stable correlated relaxation
--- is that of a Feynman roton, transplanted to a lattice gauge
theory. In helium the sound branch is softened where $S(q)$ peaks;
here the electric field is microscopically saturated,
...
cost is smallest where the Feynman ratio is smallest. We note that
the analogy extends to the $S(q)$-driven softening but, on the
present evidence, \emph{not} to Feynman--Cohen backflow: the exact
family-resolved decomposition shows the mode's relaxation is not
concentrated in the degrees of freedom the probe leaves unconstrained
(Appendix~\ref{app:backflow}).

Whether the soft feature is a \emph{minimum} deserves a precise
statement, because the pole and the centroid behave differently along
the radial direction, and conflating them invites a wrong conclusion
in each direction. The \emph{centroid} $f_1/S$ rises monotonically
from $\Gamma$ to $L$ along $[111]$ and is stationary at $L$ only by
band periodicity; as a function on the full momentum space it has a
saddle-like character there, soft tangentially, endpoint-maximal
radially. The \emph{excitation branch itself} does the opposite: at
every diagonalized size the dominant pole at the nearest radial
interior momentum lies far above the $L$ pole --- $1.90$ versus
$1.02K$ at 48 sites, $1.65$ versus $0.95K$ at 64, $1.54$ versus
$1.07K$ at 72 --- so the pole dispersion \emph{descends} into $L$
along the radial direction and, with the fold symmetry, has positive
curvature there. The centroid rises radially only because the
interior spectra are pole-dominated, with centroid close to pole,
while at $L$ the centroid is dragged upward by the near-half of the
weight that lies at high energy. On the finite clusters, then, the
branch has a genuine finite-momentum minimum at $L$ in every sampled
direction, flat along the soft valley; the saddle geometry belongs to
the first moment, not to the excitation.

Two statements remain beyond the data, and the distinction is part of
the result rather than a hedge. First, sharpness beyond 96 spins:
exact diagonalization establishes a single dominant pole up to that
size, while the Monte Carlo constrains only the centroid; whether the
pole broadens into a narrow continuum in the thermodynamic limit is
open, and the gapless theory permits multi-quantum weight to descend
toward any branch. What is size-stable is where the spectrum is soft
and how much weight is there. Second, the trivially gapless photon:
in the thermodynamic limit the excitation energy at small enough $q$
lies below any finite-energy feature, so ``lowest'' statements are
always relative to the momenta away from the immediate neighborhood
of $\Gamma$, as the finite clusters sample them.

We also state what the interpretation does not rest on. It does not
rest on photon ancestry: with $E^2$ a c-number there is no
interpolation to a free theory, so no adiabatic branch labeling
exists, and we call the mode only the dominant excitation in one
transverse $S^z$ channel. It does not rest on the fitted Gaussian
scale: the branch ratio $0.55$--$0.59$ and the zone-boundary
hierarchy of $f_1/S$ involve no fitted quantity, while every
statement of the form ``half the Gaussian energy'' does, and is so
labeled. It does not rest on the flippability census, which is a
consistent signature but not size-stable in magnitude
(Sec.~\ref{sec:census-limits}). And the polarization statements are
exact where the little group forbids mixing and approximate
elsewhere: the two dominant levels are orthogonal in polarization to
machine precision at three momenta, to $0.3\%$ at $L$ and to $4\%$ at
...
\caption{The $L$-point spectrum across all four diagonalized cluster
 ...
 $\omega_{\rm low}/\omega_{\rm high}$. The 48-site values are complete
 diagonalization; 64, 72 and 96 are Lanczos-converged
 (Appendix~\ref{app:methods}), with the equal-time sum rule saturated
 to $1.0000$ at every size.}
\label{fig:ladder}
\end{figure}
\begin{figure}[t]
\centering
...
\end{tabular}
\end{center}
in units of $K$. Quoted uncertainties are the larger of the binned
statistical error and the scatter among symmetry copies of a star
divided by the square root of their number; where the copy scatter
exceeds the binned estimate (as at $L$, whose four copies scatter by
$0.039$ against a binned $0.015$), it signals residual
autocorrelation and the inflated value is used. The minimum of the
ratio over the zone boundary sits
...
the extrapolation predicts. In the model's $S^z$ channel the lower
feature carries roughly twice the weight of the upper one. We do not
translate that ratio into a neutron intensity ratio: the physical
cross section combines the sublattice amplitudes coherently, with
material-specific local-axis and $g$-tensor coefficients, before the
modulus is squared, so the two eigenstates can be reweighted
differently and cross-sublattice interference can shift the apparent
fractions. For the same reason we do not claim that laboratory
neutron-polarization channels separate the two features; the emergent
transverse basis of Appendix~\ref{app:polarization} need not coincide
with any laboratory polarization axis. Converting the channel-level
prediction into a cross section for a specific material requires its
local frames and $g$-tensor and is left for material-specific work.
We caution finally that a finite cluster produces discrete poles
where the thermodynamic system may produce a peak plus a continuum,
so the robust prediction is the redistribution of weight, not the
pole count.
...
full microscopic spin Hilbert space. Second, resolved spectra exist at
four sizes and no more (complete at 48 sites, Lanczos-converged
above); the scale-free diagnostics --- the branch
...
gauge theory of the ice manifold, reconstructs its finite-momentum
spectrum at order one: at the zone-boundary $L$ point a single level
at $0.90$--$1.07K$ carries at least half of the longitudinal weight,
at roughly half the Gaussian-photon energy, below the dominant level
at every other accessible momentum away from the gapless
zone-center neighborhood --- on every cluster from 48 to 96 spins.
The location is fixed by Feynman's mechanism. The ground-state ratio
$f_1/S$ predicts where the spectrum reconstructs --- forward, four
times, once as a near-degeneracy prediction between different
momentum stars --- and sign-free Monte Carlo shows that its
zone-boundary minimum at $L$ survives, essentially unchanged, to 2048
sites. The relaxation that carries the mode below the single-mode
bound recovers a size-stable $28$--$32\%$ of the bare cost through a
family-selective reorganization of ring coherence, resolved exactly
by plaquette decomposition.
...
rank-two transverse curl of Appendix~\ref{app:polarization}. (The
...
Appendix~\ref{app:gfmc}, which must reach this nonzero-$P$ sector.)
...
is certified by the energy gate of Appendix~\ref{app:gfmc}, which
